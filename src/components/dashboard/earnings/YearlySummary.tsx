...
interface YearlySummaryProps {
  summary: EarningsDashboard['yearly'];
  currency: EarningsCurrency;
  onDownload: () => void;
}

const YearlySummary = ({ summary, currency, onDownload }: YearlySummaryProps) => {
  return (
    <div className="p-6 sm:p-10 rounded-3xl bg-brand-500/5 dark:bg-brand-500/10 border border-gray-200 dark:border-gray-800 shadow-2xl shadow-brand-500/5 flex flex-col sm:flex-row items-center justify-between gap-8 sm:gap-12 relative overflow-hidden group">
      ...
      <div className="space-y-3 sm:space-y-4 text-center sm:text-left z-10 w-full flex-1">
        <h2 className="text-[10px] sm:text-lg font-bold text-gray-900 dark:text-white uppercase tracking-widest opacity-80">{summary.year} Year-to-Date Summary</h2>
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
          <h3 className="text-4xl sm:text-6xl font-black text-gray-900 dark:text-brand-400 tracking-tighter">{formatEarningsCurrency(summary.yearToDate, currency)}</h3>
