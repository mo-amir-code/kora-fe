...
export default function HeroSection() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError("");
    router.push(
      `/auth/signup?email=${encodeURIComponent(email.trim())}&subscription=free-trial`
    );
  };
  ...
  return (
    <section className="relative pt-24 pb-16 sm:pt-32 sm:pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10 text-center">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-brand-500/10 dark:bg-brand-500/15 border border-brand-500/30 text-xs font-bold text-brand-600 dark:text-brand-400 mb-6 sm:mb-8 tracking-wide shadow-xs">
          ✨ #1 Sponsorship OS for Digital Creators
        </div>
        ...
        <div className="max-w-md mx-auto px-2">
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              type="submit"
              className="w-full sm:w-auto px-6 py-3.5 sm:px-8 sm:py-4 rounded-2xl bg-brand-500 hover:bg-brand-600 text-white font-bold transition-all sm:shrink-0 flex items-center justify-center gap-2 text-sm sm:text-base shadow-lg shadow-brand-500/20 active:scale-95 cursor-pointer"
            >
              Start for free
          ...
          {error && (
            <p className="mt-2 text-xs text-red-400 text-left px-2 font-medium">
              {error}
            </p>
          )}
        </div>
